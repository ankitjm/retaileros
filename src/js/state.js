// Check localStorage for existing login status — require token to be present too
const savedToken = localStorage.getItem('retaileros_session_token');
...
// If token is missing, clear stale login flags so the login page shows
const savedLoginStatus = localStorage.getItem('retaileros_logged_in') === 'true' && !!savedToken && !!savedRetailerId;
if (!savedLoginStatus) {
...
export const state = {
    ...
    salesBillingMode: 'retail', // retail | bulk
    selectedNewsId: null  // currently selected news article ID
};
...
window.setSelectedClient = (name) => setClientMode('profile', name);
...
export function setSalesBillingMode(mode) {
    state.salesBillingMode = mode;
    triggerRender();
}
window.setSalesBillingMode = setSalesBillingMode;

export function setSelectedNews(id) {
    state.selectedNewsId = id;
    triggerRender();
}
window.setSelectedNews = setSelectedNews;

export function setInventoryBrand(brand) {
    state.inventoryBrand = brand;
    triggerRender();
}
...
window.setInventoryBrand = setInventoryBrand;
