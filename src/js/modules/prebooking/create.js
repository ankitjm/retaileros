...
// Publish campaign — writes to DB, then navigates back
window._publishCampaign = async () => {
    const title = document.getElementById('pb-title')?.value?.trim();
    const product = document.getElementById('pb-product')?.value?.trim();
    const deposit = parseFloat(document.getElementById('pb-deposit')?.value?.trim()) || 0;
    const duration = parseInt(document.getElementById('pb-duration')?.value?.trim()) || 14;
    ...
    if (!product) {
        if (window.toast) window.toast.error('Select a product for this campaign');
        return;
    }
    ...
    try {
        ...
        // Update cache
        if (!window._db_cache.campaigns) window._db_cache.campaigns = [];
        window._db_cache.campaigns.unshift({ id, title, product, deposit_amount: deposit, start_date: startDate, end_date: endDate, status: 'active' });
        ...
        console.error('[PreBooking] publish error:', err);
        if (window.toast) window.toast.error('Failed to publish campaign');
    }
};
