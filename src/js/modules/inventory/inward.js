...
export async function confirmInward() {
    ...
    if (!model || !imei) {
        window.toast?.warning("Model name and IMEI are required.") || alert("Model name and IMEI are required.");
        return;
    }
    ...
    try {
        ...
        if (existing.length > 0) {
            ...
            // Use updateStock helper to increment by 1
            await db.inventory.updateStock(existing[0].id, 1);
        } else {
            productId = 'PRD-' + Math.random().toString(36).substr(2, 6).toUpperCase();
            await db.inventory.addProduct({
                id: productId,
                name: model,
                category: 'Smartphone',
                brand: model.split(' ')[0],
        ...
        await syncData();
        window.toast?.success("Inward recorded successfully!");
        if (window.setInventoryMode) window.setInventoryMode('details');
    } catch (err) {
        window.toast?.error("Inward failed: " + err.message) || alert("Inward failed: " + err.message);
        btn.disabled = false;
