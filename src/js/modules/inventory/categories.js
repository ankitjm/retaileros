export function renderCategories() {
    ...
    const categoryStats = products.reduce((acc, p) => {
        ...
        const stock = parseInt(p.in_stock) || 0;
        acc[cat].count += stock;

        const price = parseFloat(p.price) || 0;
        const mop = parseFloat(p.mop) || price;
        acc[cat].blocked += stock * price;
        acc[cat].margin += (mop - price) * stock;
    ...
    return `
        ...
        return `
                <div onclick="setInventoryCategory('${cat.replace(/'/g, "\\'")}'); setInventoryMode('details')" class="card p-5 border-2 transition-all cursor-pointer group bg-white flex items-center justify-between border-transparent hover:border-slate-200 text-left">
