...
window._reassignRepair = async (memberName) => {
    ...
    try {
        await db.repairs.updateAssignee(state.activeRepairId, memberName);
        // Update cache immediately
        const cache = window.getCache ? window.getCache() : {};
        const repair = (cache.repairs || []).find(r => r.id === state.activeRepairId);
        if (repair) repair.assigned_to = memberName;
        if (window.toast) window.toast.success(`Reassigned to ${memberName}`);
    } catch (err) {
        console.error('Reassign error:', err);
        if (window.toast) window.toast.error('Failed to reassign repair');
    }
    window.triggerRender();
};
