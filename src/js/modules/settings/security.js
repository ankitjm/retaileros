    ...
    return `
        <div data-settings-category="security" class="h-full flex flex-col relative bg-white animate-slide-up text-left">
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar text-left">
                ...
                <div class="p-6 space-y-4 border-b border-dashed border-slate-200 text-left">
                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 text-left">
                        <span class="w-1.5 h-1.5 bg-slate-400 rounded-full"></span> Active Session
                    </p>
                    <div class="space-y-3 text-left">
                        <div class="card p-4 flex items-center justify-between text-left">
                            <div class="flex items-center gap-3 text-left">
                                <div class="w-10 h-10 bg-slate-900 rounded-xl flex items-center justify-center">
                                    <span class="material-icons-outlined text-white">${/Mobi|Android/i.test(navigator.userAgent) ? 'smartphone' : 'computer'}</span>
                                </div>
                                <div class="text-left">
                                    <p class="text-xs font-black text-slate-900">This Device</p>
                ...
                <!-- Login History from activity logs -->
                    ...
                    <div class="space-y-2 text-left">
                        ${(() => {
                            ...
                            const logs = (cache.activityLogs || [])
                                .filter(l => l.action === 'login' || l.action === 'auth')
                                .slice(0, 10);
                            if (logs.length === 0) {
                                return `<p class="text-[10px] text-slate-300 text-center py-4">No login history yet</p>`;
                            }
                            return logs.map(l => {
                                const time = l.created_at ? new Date(l.created_at).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' }) : '';
                                return `
                                    <div class="flex items-center justify-between py-3 border-b border-slate-50 text-left">
                                        <div class="text-left">
                                            <p class="text-[10px] font-bold text-slate-900">${l.user_name || l.detail || 'Login'}</p>
                                            <p class="text-[8px] font-bold text-slate-400">${time}</p>
                                        </div>
                                        <span class="text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded-full bg-slate-900 text-white">success</span>
                                    </div>
                                `;
                            }).join('');
                        })()}
