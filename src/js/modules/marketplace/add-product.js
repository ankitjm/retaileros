...
let _mpCondition = 'new';

window._setMpCondition = (val) => {
    _mpCondition = val;
    document.querySelectorAll('[data-mp-cond]').forEach(btn => {
        if (btn.dataset.mpCond === val) {
            btn.className = 'flex-1 py-3 bg-white shadow-sm text-slate-950 text-[9px] font-black uppercase rounded-xl transition-all text-center';
        } else {
            btn.className = 'flex-1 py-3 text-slate-400 text-[9px] font-black uppercase rounded-xl transition-all text-center';
        }
    });
};
...
window._postToMarketplace = async () => {
    ...
    try {
        ...
        const retailer = cache.retailers?.find(r => r.id === localStorage.getItem('retaileros_retailer_id')) || {};
        const product = (cache.products || []).find(p => p.name === productName);

        await db.marketplace.add({
            id,
            product_name: productName,
            brand: product?.brand || productName.split(' ')[0],
            category: product?.category || null,
            price,
            quantity,
            condition: _mpCondition || 'new',
            seller_name: retailer.retailer_name || retailer.contact_person || 'Retailer',
            seller_phone: retailer.mobile_number || '',
            status: 'active',
                ...
                <div class="space-y-6 text-left">
                    ...
                    <div>
                        ...
                        <div class="flex bg-slate-100 p-1 rounded-2xl gap-1">
                            <button data-mp-cond="new" onclick="window._setMpCondition('new')" class="flex-1 py-3 bg-white shadow-sm text-slate-950 text-[9px] font-black uppercase rounded-xl transition-all text-center">Sealed</button>
                            <button data-mp-cond="open_box" onclick="window._setMpCondition('open_box')" class="flex-1 py-3 text-slate-400 text-[9px] font-black uppercase rounded-xl transition-all text-center">Open Box</button>
                            <button data-mp-cond="used" onclick="window._setMpCondition('used')" class="flex-1 py-3 text-slate-400 text-[9px] font-black uppercase rounded-xl transition-all text-center">Used</button>
                        </div>
                    </div>
