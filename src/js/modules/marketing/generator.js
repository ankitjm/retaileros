...
// Generate creative — all OpenAI calls proxy through /api/openai
window._generateCreative = async () => {
    ...
    try {
        ...
        // If reference images, use vision endpoint to enhance prompt
        if (mState.uploadedImages.length > 0) {
            const visionResponse = await fetch((window._apiBase||'')+'/api/openai/vision', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', ...getAuthHeader() },
                body: JSON.stringify({
                    prompt: `You are a marketing creative expert for an Indian electronics retail store. The user wants to create a marketing creative with this prompt: "${prompt}". Analyze the reference image and create an enhanced, detailed DALL-E prompt that incorporates visual elements from it. Keep the Indian retail context. Output ONLY the enhanced prompt, nothing else. Max 200 words.`,
                    imageBase64: mState.uploadedImages[0].base64
                })
            });

            if (visionResponse.ok) {
                const visionData = await visionResponse.json();
                enhancedPrompt = visionData.text || prompt;
            }
        }

        // Generate image with DALL-E 3 via server proxy
        ...
        const dalleResponse = await fetch((window._apiBase||'')+'/api/openai/generate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', ...getAuthHeader() },
            body: JSON.stringify({
                prompt: dallePrompt,
        ...
        if (!dalleResponse.ok) {
            ...
            throw new Error(errData.error || `API error: ${dalleResponse.status}`);
        }
