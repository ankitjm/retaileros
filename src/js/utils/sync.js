...
export async function syncData() {
    console.group("🔄 Synchronizing Data...");

    // Skip all queries if user isn't logged in — avoids 401 reload loop
    ...
    try {
        ...
        const [customers, products, sales, saleItems, companies, groups, groupMembers, automations, automationMessages, communications, schemes, retailers, inquiries, repairs, inventoryLogs, retailerSettingsRaw, teamMembers, teamRoles, retailerPlugins, activityLogs, storeListings, storeOrders, storeOrderItems, campaigns, marketplaceListings, prebookings, expenses, brandTargets] = await Promise.all([
            ...
            tenantQuery("sale_items").catch(e => { console.error("Sync sale_items failed:", e); return []; }),
            ...
            // TENANT-SCOPED: campaigns, marketplace, prebookings
            tenantQuery("campaigns", "created_at DESC").catch(e => { console.error("Sync campaigns failed:", e); return []; }),
            query("SELECT * FROM marketplace_listings WHERE status = 'active' ORDER BY created_at DESC").catch(e => { console.error("Sync marketplace failed:", e); return []; }),
            tenantQuery("prebookings", "created_at DESC").catch(e => { console.error("Sync prebookings failed:", e); return []; }),
            // TENANT-SCOPED: expenses, brand_targets
            tenantQuery("expenses", "date DESC").catch(e => { console.error("Sync expenses failed:", e); return []; }),
            tenantQuery("brand_targets", "brand").catch(e => { console.error("Sync brand_targets failed:", e); return []; })
        ]);
...
window.getCache = () => window._db_cache || {
    ...
    marketplace: [], campaigns: [], prebookings: [], expenses: [], brandTargets: [], bookings: []
};
